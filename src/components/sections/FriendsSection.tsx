...
import { Users, MessageSquare, UserCheck, UserX, Clock, Heart, ChevronLeft, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="p-6 md:p-10 space-y-10 max-w-[1400px]">
          ...
          <div className="p-8 md:p-12 relative z-10 flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-2xl md:text-3xl font-medium text-white leading-tight">
                You and <span className="font-extrabold text-white">{bffName}</span> exchanged <span className="font-extrabold text-white">{bff.messageCount.toLocaleString()}</span> messages.
              </p>
            </div>
          </div>
        </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {topFriends.length > 0 && (
          ...
          >
            <div className="p-6 border-b border-[#252B34]">
              ...
              <p className="text-[#9DA7B3] text-xs mt-1">Ranking by total direct messages sent.</p>
            </div>
              ...
              {topFriends.map((f, i) => {
                ...
                return (
                  <motion.div
                    ...
                    transition={{ delay: 0.25 + i * 0.02, ease: [0.16, 1, 0.3, 1] }}
                  >
        ...
        {friends.length > 0 && (
          <motion.div
            className="premium-card flex flex-col overflow-hidden self-start"
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex-1">
              {paginatedFriends.map((rel, i) => {
                ...
                return (
                  <div
                    key={rel.id}
                    className={`flex items-center gap-3 px-5 py-2.5 hover:bg-[#12151A]/60 transition-colors cursor-default ${i !== paginatedFriends.length - 1 ? 'border-b border-[#252B34]/40' : ''}`}
                  >
                    <div className="w-7 h-7 rounded-full bg-[#171B21] border border-[#252B34] flex items-center justify-center flex-shrink-0 text-[#7C8CFF] text-[11px] font-bold font-sans">
                      {name[0]?.toUpperCase() ?? "?"}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-white text-[13px] font-semibold truncate leading-tight font-sans">{name}</p>
                    </div>
                    <span className="text-[#5E6976] text-[11px] font-mono flex-shrink-0">@{rel.user.username}</span>
                  </div>
                );
              })}
            </div>
